  ...
  return (
    <section id="contact" ref={ref} class="bg-bg">
      <div class="max-w-[1100px] mx-auto py-28 px-6">
        <span class="font-mono text-[11px] tracking-[3px] uppercase text-sa block mb-2.5 reveal">{text(locale.value, 'Hubungi Saya', 'Get In Touch')}</span>
        <h2 class="text-[clamp(26px,4vw,40px)] font-extrabold text-ink mb-3.5 leading-[1.15] reveal d-100">{text(locale.value, 'Hubungi Saya', 'Contact Me')}</h2>
        ...
        <div class="grid grid-cols-2 gap-[60px] items-start max-[900px]:grid-cols-1">
          <div class="reveal-l d-200">
            <p class="text-[15.5px] text-ink-2 leading-[1.8] mb-8">
              {locale.value === 'id'
                ? 'Apakah Anda memiliki proyek yang ingin dikerjakan bersama, atau sekadar ingin berdiskusi tentang teknologi? Jangan ragu untuk menghubungi saya — saya selalu terbuka untuk peluang baru dan kolaborasi yang menarik.'
                : 'Have a project to work on together, or just want to discuss technology? Feel free to contact me. I am always open to new opportunities and meaningful collaboration.'}
            </p>

            <div class="flex flex-col gap-3.5">
              <a href="mailto:randi@starnsparks.com" class="flex items-center gap-3.5 p-4 px-5 bg-surf border border-bdr rounded-xl no-underline text-ink transition-all duration-300 hover:border-sa hover:translate-x-1.5 hover:shadow-[0_8px_24px_rgb(var(--sa)/0.15)]">
                ...
                <div>
                  ...
                  <div class="text-sm text-ink font-medium mt-0.5">randi@starnsparks.com</div>
                </div>
              </a>
              ...
              <a href="https://wa.me/6282110474856" target="_blank" rel="noreferrer" class="flex items-center gap-3.5 p-4 px-5 bg-surf border border-bdr rounded-xl no-underline text-ink transition-all duration-300 hover:border-sa hover:translate-x-1.5 hover:shadow-[0_8px_24px_rgb(var(--sa)/0.15)]">
                <div class="w-[42px] h-[42px] bg-sa/12 rounded-xl flex items-center justify-center shrink-0 [&>svg]:w-5 [&>svg]:h-5 [&>svg]:text-sa">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
                ...
                <div>
                  <div class="text-[11px] text-ink-3 uppercase tracking-[1px] font-mono">WhatsApp</div>
              ...
              <div class="flex items-center gap-3.5 p-4 px-5 bg-surf border border-bdr rounded-xl text-ink" style="cursor:default;">
                ...
                <div>
                  <div class="text-[11px] text-ink-3 uppercase tracking-[1px] font-mono">{text(locale.value, 'Lokasi', 'Location')}</div>
          ...
          <div class="reveal-r d-300">
            <div class="text-[15px] font-bold text-ink mb-5">{text(locale.value, 'Kirim Pesan', 'Send Message')}</div>

            <div class="flex flex-col gap-3.5">
              <div class="grid grid-cols-2 gap-3">
                <div class="flex flex-col gap-1.5">
                  <label class="text-[11px] text-ink-3 uppercase tracking-[1px] font-mono">{text(locale.value, 'Nama *', 'Name *')}</label>
                  <input
                    type="text"
                    placeholder={text(locale.value, 'Nama lengkap', 'Full name')}
                    value={name.value}
                ...
                <div class="flex flex-col gap-1.5">
                  <label class="text-[11px] text-ink-3 uppercase tracking-[1px] font-mono">{text(locale.value, 'No. WhatsApp', 'WhatsApp Number')}</label>
                  <input
                    type="tel"
              ...
              <div class="flex flex-col gap-1.5">
                <label class="text-[11px] text-ink-3 uppercase tracking-[1px] font-mono">{text(locale.value, 'Pesan *', 'Message *')}</label>
                <textarea
                  rows={4}
                  placeholder={text(locale.value, 'Ceritakan proyek atau kebutuhan Anda...', 'Tell me about your project or needs...')}
                  value={message.value}
              ...
              >
                {sent.value ? (
                  <>
                    ...
                    {text(locale.value, 'Pesan Terkirim!', 'Message Sent!')}
                  </>
                ) : (
                  <>
                    ...
                    {text(locale.value, 'Kirim via WhatsApp', 'Send via WhatsApp')}
                  </>
                )}
              </button>
            </div>
